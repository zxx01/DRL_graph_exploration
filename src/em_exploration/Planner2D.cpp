...
namespace em_exploration
{
	...
	void EMPlanner2D::updateNodeInformation_EM(Node::shared_ptr node)
	{
		...
		if (parameter_.dubins_control_model_enabled)
		{
			for (int step = 0; step < node->poses.size(); ++step)
			{
				...
				SLAM2D::OdometryFactor2DPtr dubins_path_factor = SLAM2D::buildOdometryFactor(parent_key, node_key, control_state);
		...
		else
		{
			...
			SLAM2D::OdometryFactor2DPtr odometry_factor = SLAM2D::buildOdometryFactor(parent_key, node_key, control_state);
		...
		Simulator2D::MeasurementVector measurements = Simulator2D::measure(*map_, node->state.pose, sensor_model_, false, false);
	...
	void EMPlanner2D::initialize(const SLAM2D &slam, const VirtualMap &virtual_map)
	{
		...
		distance_weight_ = parameter_.distance_weight0 - (parameter_.distance_weight0 - parameter_.distance_weight1) * percentage_known;
		//  }
