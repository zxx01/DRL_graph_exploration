...
namespace em_exploration
{
  ...
  double VirtualMap::explored() const
  {
    ...
    {
      if ((virtual_landmarks_[i].probability < 0.49 || virtual_landmarks_[i].probability > 0.6) && parameter_.getMinX() + extg <= virtual_landmarks_[i].point.x() && virtual_landmarks_[i].point.x() <= parameter_.getMaxX() - extg && parameter_.getMinY() + extg <= virtual_landmarks_[i].point.y() && virtual_landmarks_[i].point.y() <= parameter_.getMaxY() - extg)
      {
        count++;
      }
    }
